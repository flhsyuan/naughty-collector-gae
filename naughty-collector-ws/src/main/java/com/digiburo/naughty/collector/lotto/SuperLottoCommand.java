 ...
 */
public class SuperLottoCommand extends AbstractLottoCommand {
  private final Logger logger = Logger.getLogger(getClass().getName());

  private final DataBaseFacade dataBaseFacade = new DataBaseFacade();

  private SuperLotto lineParser(final String raw) {
...
//    System.out.println("lines:" + buffer.length);
    ...
    for (int ii = 0; ii < buffer.length; ii++) {
      SuperLotto superLotto = lineParser(buffer[ii]);
//      System.out.println(superLotto);
      if (superLotto != null) {
        SuperLotto selected = dataBaseFacade.selectSuperLotto(superLotto.getDraw());
        if (selected == null) {
          ++success;
          dataBaseFacade.save(superLotto);
        }
      }
    }

    dataBaseFacade.writeLog(LogFacility.LOTTO, LogPriority.INFO, "SuperLotto load summary:" + buffer.length + ":" + success);
  }

  public void execute() {
    String raw = fetcher(SUPER_LOTTO_URL);
    if (!raw.isEmpty()) {
      rawParser(raw);
    }
  }
